...
Poltergeist.Browser = (function() {
  ...
  Browser.prototype.pushFrame = function(command, name, timeout) {
    var frame, frame_url;
    this.debug("pushFrame: name=" + name + " timeout=" + timeout + " ");
    ...
    this.debug("frameUrl is " + frame_url);
    if (indexOf.call(this.currentPage.blockedUrls(), frame_url) >= 0) {
      ...
      this.debug("currentUrl is " + (this.currentPage.currentUrl()));
      if (frame_url && (frame_url !== 'about:blank') && (this.currentPage.currentUrl() === 'about:blank')) {
        this.debug("waiting for frame load - initial state = " + this.currentPage.state + " setting to 'awaiting_frame_load'");
        this.currentPage.state = 'awaiting_frame_load';
        return this.currentPage.waitState('default2', (function(_this) {
          return function() {
            _this.debug("page state became 'default'");
            return command.sendResponse(true);
          };
        })(this), 3, (function(_this) {
          return function() {
            return _this.debug("Frame state hasn't become 'default' in 3 seconds stuck at - " + _this.currentPage.state + " - open resources are " + (_this.currentPage.openResourceRequests().join(',')));
          };
        })(this));
      } else {
      ...
      this.debug("failed to push frame");
      if (new Date().getTime() < timeout) {
        this.debug("delaying a bit");
  ...
  Browser.prototype.push_frame = function(name, timeout) {
    ...
    this.debug("push_frame - current state is " + this.currentPage.state + " for page " + this.currentPage.id);
